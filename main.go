// Command llvm-apc builds and smoke-tests the actions-precompiled LLVM distribution.
//
//	go run . --list-to-build
//	go run . --dry-run trunk
//	go run . trunk
package main
...
func (llvmPackage) Meta() foundation.Meta {
	return foundation.Meta{
		...
		// CI matrix sets APC_TARGETS per runner; local defaults to host OS target.
		DefaultTargets: []string{defaultHostTarget()},
	}
}
...
// buildLinuxDocker runs the kitchen-sink build script inside the package image.
// Extra LLVM_* knobs are forwarded from the host env (no templating).
func buildLinuxDocker(ctx context.Context, deps foundation.Deps, meta foundation.Meta, req foundation.BuildRequest) error {
	if deps.Docker == nil {
		return fmt.Errorf("docker client is nil")
	}
	imageTag := firstNonEmpty(deps.Env.Get(foundation.EnvImageTag), "local")
	imageName := firstNonEmpty(deps.Env.Get(foundation.EnvImageName), meta.ImageName)
	...
	env := map[string]string{
		...
		"LANG":            "C.UTF-8",
		"LC_ALL":          "C.UTF-8",
		"LLVM_VERSION":    req.Version,
	}
	// Forward optional knobs if set on the host (CI or local).
	...
	return deps.Docker.Run(ctx, foundation.DockerRunRequest{
		Image: image,
		User:  foundation.HostDockerUser(),
...
func buildWindowsNative(ctx context.Context, deps foundation.Deps, meta foundation.Meta, req foundation.BuildRequest) error {
	script := filepath.Join(deps.WorkDir, "scripts", "build_windows.ps1")
	if _, err := deps.FS.Stat(script); err != nil {
		return fmt.Errorf("windows build script missing: %w", err)
	}
	...
	envExtra := map[string]string{
		...
		"PACKAGE_NAME":    meta.Name,
		"BUILD_TARGET":    req.Target,
		"OUTPUT_DIR":      req.OutDir,
		"UPSTREAM_REPO":   meta.UpstreamGit,
	}
	// Merge into process env for the runner
	base := deps.Env.Environ()
	for k, v := range envExtra {
		base = setEnvList(base, k, v)
	}
	if rw, ok := deps.Runner.(foundation.RunnerWithOpts); ok {
		return rw.RunWith(ctx, foundation.RunOpts{Env: base, Dir: deps.WorkDir},
			"pwsh", "-NoProfile", "-File", script)
	}
	return deps.Runner.Run(ctx, "pwsh", "-NoProfile", "-File", script)
}
